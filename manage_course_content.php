        ...
        case 'remove_content':
            ...
            if ($content_id > 0 && in_array($content_type, ['category', 'subcategory', 'post'])) {
                try {
                    ...
                    ");
                    $result = $stmt->execute([$course_id, $content_type, $content_id]);
...
if ($training_tables_exist && $course && $course_content_table_exists) {
    try {
        // First, check what columns actually exist in the training_course_content table
        $columns_check = $pdo->query("SHOW COLUMNS FROM training_course_content");
        $columns = [];
        while ($row = $columns_check->fetch(PDO::FETCH_ASSOC)) {
            $columns[] = $row['Field'];
        ...
        if (in_array('added_by', $columns)) {
            $select_clause .= ", u.name as added_by_username";
            $join_clause = "LEFT JOIN users u ON tcc.added_by = u.id";
        } else {
            $join_clause = "";
        }

        // Check if added_at column exists, otherwise use a fallback
        $order_clause = "ORDER BY tcc.content_type";
        if (in_array('added_at', $columns)) {
            $select_clause .= ", tcc.added_at";
            $order_clause .= ", tcc.added_at DESC";
        } else {
            $select_clause .= ", CURRENT_TIMESTAMP() as added_at";
            $order_clause .= ", tcc.id DESC";
        }

        $stmt = $pdo->prepare("
            SELECT {$select_clause},
                   CASE tcc.content_type
                       WHEN 'category' THEN (SELECT name FROM categories WHERE id = tcc.content_id)
                       WHEN 'subcategory' THEN (SELECT name FROM subcategories WHERE id = tcc.content_id)
                       WHEN 'post' THEN (SELECT title FROM posts WHERE id = tcc.content_id)
                   END as content_name
...
<head>
    ...
    <title>Manage Course Content - <?php echo htmlspecialchars($course['name'] ?? 'Course'); ?></title>
...
<body>
    <div class="container">
        ...
        <?php else: ?>
            ...
            <div class="content-section">
                <div class="section-header">📋 Current Course Content (<?php echo count($course_content); ?> items)</div>
                <div class="section-content">
                                ...
                                <div class="content-item">
                                    <div class="actions">
                                        <form method="POST" action="" style="margin: 0;">
                                            ...
                                            <input type="hidden" name="content_type" value="<?php echo $content['content_type']; ?>">
                                            <input type="hidden" name="content_id" value="<?php echo $content['content_id']; ?>">
                                            <button type="submit" class="btn btn-danger btn-sm"
                                                    onclick="return confirm('Are you sure you want to remove this content from the course?')">
                                                🗑️ Remove
                                            </button>
                                        </form>
                                    </div>
                                    <span class="content-type-badge badge-<?php echo $content['content_type']; ?>">
                                        <?php echo $content['content_type']; ?>
                                    </span>
                                    <h4><?php echo htmlspecialchars($content['content_name'] ?? 'Unknown Content'); ?></h4>
                                    <div class="meta">
                                        Added by <?php echo htmlspecialchars($content['added_by_username'] ?? 'Unknown'); ?>
                                        on <?php echo date('M j, Y', strtotime($content['added_at'])); ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
