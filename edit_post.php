                ...
                for ($i = 0; $i < $file_count; $i++) {
                    if ($_FILES['files']['error'][$i] == UPLOAD_ERR_OK) {
                        ...
                        if (move_uploaded_file($tmp_name, $file_path)) {
                            // Insert file record
                            $stmt = $pdo->prepare("
                                INSERT INTO files (post_id, original_filename, stored_filename, file_path, file_size, file_type)
                                VALUES (?, ?, ?, ?, ?, ?)
                            ");
    ...
    <?php else: ?>
        <div class="card">
            ...
            <form method="POST" action="edit_post.php?id=<?php echo $post_id; ?>" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="files" class="form-label">Attach New Files (Optional)</label>
                    <input
                        type="file"
                        id="files"
                        name="files[]"
                        ...
                        accept="*/*"
                    >
                    <div class="form-hint">Max 20 MB per file. You can select multiple files.</div>
                </div>
