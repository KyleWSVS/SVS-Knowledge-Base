...
function get_overall_training_progress($pdo, $user_id) {
    try {
        // Simplified quiz-based progress calculation
        $stmt = $pdo->prepare("
            SELECT
                COUNT(DISTINCT tq.id) as total_items,
                COUNT(DISTINCT CASE WHEN tp.quiz_completed = TRUE THEN tq.id END) as completed_items,
                COUNT(DISTINCT CASE WHEN tp.quiz_completed = FALSE AND tp.status = 'in_progress' THEN tq.id END) as in_progress_items,
                0 as total_courses,
                0 as completed_courses
            FROM training_quizzes tq
            LEFT JOIN training_progress tp ON tq.id = (
                SELECT tq2.id FROM training_quizzes tq2
                WHERE tq2.content_id = tp.content_id
                AND tq2.content_type = tp.content_type
                AND tq2.is_active = TRUE
                LIMIT 1
            ) AND tp.user_id = ?
            WHERE tq.is_active = TRUE
            AND (
                -- Include if user is training role (can access all quizzes)
                (SELECT 1 FROM users WHERE id = ? AND role = 'training')
                -- OR include if user is formally assigned to training containing this content
                OR EXISTS (
                    SELECT 1 FROM user_training_assignments uta
                    JOIN training_course_content tcc ON uta.course_id = tcc.course_id
                    WHERE uta.user_id = ?
                    AND tcc.content_type = tq.content_type
                    AND tcc.content_id = tq.content_id
                    AND uta.status != 'completed'
                )
            )
        ");
...
function promote_user_if_training_complete($pdo, $user_id) {
    try {
        ...
        if ((int)$data['total_courses'] > 0 && (int)$data['incomplete_courses'] === 0) {
            ...
            ");
            return $update_stmt->execute([$user_id]);
        }
...
function revert_user_to_training($pdo, $user_id, $course_id, $reason) {
    try {
        ...
        // Reset course assignment status
        ...
        // Reset progress for affected course content
        $progress_stmt = $pdo->prepare("
